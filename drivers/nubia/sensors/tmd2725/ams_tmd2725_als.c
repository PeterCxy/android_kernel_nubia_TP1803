...
static int tmd2725_read_als(struct tmd2725_chip *chip)
{
	...
	ret = ams_i2c_blk_read(chip->client, TMD2725_REG_CH0DATA,
			       &chip->shadow[TMD2725_REG_CH0DATA],
			       4 * sizeof(u8));
	...
	if (ret >= 0) {
		chip->als_inf.als_ch0 =
		    le16_to_cpu(*
				((const __le16 *)&chip->
				 shadow[TMD2725_REG_CH0DATA]));
		chip->als_inf.als_ch1 =
		    le16_to_cpu(*
				((const __le16 *)&chip->
				 shadow[TMD2725_REG_CH1DATA]));
		ret = 0;
	}
	...
	if (state)		// Turning on ALS
	{
		...
		if (chip->prox_enabled)
			ams_i2c_modify(chip, sh, TMD2725_REG_ENABLE,
				       TMD2725_WEN | TMD2725_AEN | TMD2725_PEN |
				       TMD2725_PON,
...
static int tmd2725_als_set_enable(struct tmd2725_chip *chip, u8 enable,
				  bool enable_poll)
{
	int ret;
	if (enable) {
		...
		if (!chip->pdata->power_always_on) {
			chip->pdata->power_state &= ~POWER_ALS_ON;
			if (!chip->prox_enabled && !chip->als_enabled
			    && chip->pdata->power_state == POWER_OFF)
...
static int tmd2725_get_lux(struct tmd2725_chip *chip)
{
	...
	if (chip->pdata->debug_level) {
		SENSOR_LOG_INFO
		    ("lux:%d [ch0:%lld, ch1:%lld, lux1:%lld, lux2:%lld]\n", lux,
		     ch0, ch1, lux1, lux2);
		SENSOR_LOG_INFO
		    ("d_factor:%lld seg[0][ch0_coef:%lld ch1_coef:%lld] seg[1][ch0_coef:%lld ch1_coef:%lld]\n",
		...
		SENSOR_LOG_INFO
		    ("cpl:%d [ATIME:%d, ((gain:%d*CYCLE:%d) / 1000)] (sat:%d)\n",
		     chip->als_inf.cpl, chip->shadow[TMD2725_REG_ATIME],
...
static int tmd2725_update_als(struct tmd2725_chip *chip)
{
	...
	if (chip->als_cal_data.flag) {
		chip->als_inf.cal_lux =
		    chip->als_inf.prev_lux *
		    SCALE_FACTOR(chip->als_cal_data.base.lux,
				 chip->als_cal_data.cur.lux);
		if (chip->pdata->debug_level)
			SENSOR_LOG_INFO
			    ("cal_lux:%d lux:%d [base:cur]=[%d:%d]\n",
...
static void tmd2725_report_als(struct tmd2725_chip *chip)
{
	if (chip->als_cal_data.flag) {
		SENSOR_LOG_DEBUG_IF(chip->pdata->debug_level, "cal_lux:%d\n",
				    chip->als_inf.cal_lux + 1);
		input_report_rel(chip->a_idev, REL_X,
				 chip->als_inf.cal_lux + 1);
	} else {
...
static void tmd2725_als_report_work(struct work_struct *work)
{
	struct tmd2725_chip *chip =
	    container_of((struct delayed_work *)work, struct tmd2725_chip,
			 als_work);
	tmd2725_update_als(chip);
	tmd2725_report_als(chip);
	schedule_delayed_work(&chip->als_work,
			      msecs_to_jiffies(chip->als_poll_delay));
}
...
static void tmd2725_config_tp_parameter(struct tmd2725_chip *chip)
{
	...
	for (i = 0; i < MODULE_MANUFACTURE_NUMBER; i++) {
		if (chip->params.panel_id ==
		    tp_module_parameter[i].tp_module_id) {
			chip->params.d_factor =
			    tp_module_parameter[i].tp_als_param.d_factor;
			chip->params.which_seg =
			    tp_module_parameter[i].tp_als_param.which_seg;
			chip->params.lux_segment[0].ch0_coef =
...
static int tmd2725_als_calibrate_work(struct tmd2725_chip *chip,
				      const char *cal_data)
{
	int err = 0;
	...
	memcpy(&chip->als_cal_data.base, cal_data,
	       sizeof(chip->als_cal_data.base));
	memcpy(&chip->als_cal_data.cur, cal_data,
	       sizeof(chip->als_cal_data.cur));
	...
	chip->als_cal_data.cur.lux =
	    (chip->als_cal_data.cur.lux > 0) ? chip->als_cal_data.cur.lux : 1;
	...
	if (chip->als_cal_data.base.lux / chip->als_cal_data.cur.lux > 6) {
		...
		SENSOR_LOG_ERROR
		    ("calibrate param invalid; data->rgb_cal_data.flag=%d\n",
				    ...
				    struct device_attribute *attr, char *buf)
{
	...
	if (!chip->als_enabled) {
		SENSOR_LOG_INFO
		    ("disabled, need enable and waiting for updating data\n");
				       ...
				       const char *buf, size_t size)
{
	...
	if (5 !=
	    sscanf(buf, "%10lld,%10lld,%10lld,%10lld,%10lld", &d_factor,
					...
					char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "%d\n",
			(((chip->
			   shadow[TMD2725_REG_PERS]) & TMD2725_MASK_APERS)));
}
...
struct device_attribute tmd2725_als_attrs[] = {
	...
	__ATTR(dev_init, 0664, tmd2725_als_dev_init_show,
	       tmd2725_als_dev_init_store),
	__ATTR(chip_name, 0444, tmd2725_als_chip_id_show, NULL),
	__ATTR(calibrate, 0644, tmd2725_als_calibrate_show,
	       tmd2725_als_calibrate_store),
	__ATTR(fac_calibrate, 0664, tmd2725_als_fac_calibrate_show,
	       tmd2725_als_fac_calibrate_store),
	__ATTR(als_Itime, 0644, tmd2725_als_itime_show,
	       tmd2725_als_itime_store),
	__ATTR(als_Wtime, 0644, tmd2725_als_wtime_show,
	       tmd2725_als_wtime_store),
	__ATTR(als_gain, 0644, tmd2725_als_gain_show, tmd2725_als_gain_store),
	__ATTR(als_cpl, 0444, tmd2725_als_cpl_show, NULL),
	__ATTR(als_thresh_deltaP, 0644, tmd2725_als_deltaP_show,
	       tmd2725_als_deltaP_store),
	__ATTR(als_auto_gain, 0644, tmd2725_auto_gain_enable_show,
	       tmd2725_auto_gain_enable_store),
	__ATTR(als_lux_table, 0644, tmd2725_lux_table_show,
	       tmd2725_lux_table_store),
	__ATTR(als_persist, 0644, tmd2725_als_persist_show,
	       tmd2725_als_persist_store),
...
static int tmd2725_als_parse_dt(struct device *dev)
{
	...
	for (i = 0; i < tp_moudle_count; i++) {
		...
		if (array_len != PARSE_DTSI_NUMBER) {
			SENSOR_LOG_ERROR
			    ("tmd2725,length invaild or dts number is larger than:%d\n",
			     array_len);
			return -array_len;
		}
		SENSOR_LOG_INFO
		    ("read lux cal parameter count from dtsi  is %d\n",
		     array_len);
		...
		for (index = 0; index < array_len; index++) {
			retval =
			    of_property_read_string_index(np, dts_array_name[i],
							  index,
							  &raw_data0_dts);
			if (retval) {
				SENSOR_LOG_ERROR
				    ("read index = %d,raw_data0_dts = %s,retval = %d error,\n",
				     index, raw_data0_dts, retval);
				return -retval;
			}
			ptr[index] = simple_strtoul(raw_data0_dts, NULL, 10);
			SENSOR_LOG_DEBUG
			    ("lux cal parameter from dtsi  is %ld\n",
			     ptr[index]);
		}
	}
...
int tmd2725_als_device_register(struct tmd2725_chip *chip,
				struct i2c_driver *driver)
{
	...
	chip->als_dev =
	    device_create(als_class, 0, tmd2725_als_dev_t, driver,
			  DEV_ALS_NAME);
	...
	ret =
	    sensor_create_sysfs_interfaces(chip->als_dev, tmd2725_als_attrs,
					   ARRAY_SIZE(tmd2725_als_attrs));
	if (ret < 0) {
...
void tmd2725_als_device_unregister(struct tmd2725_chip *chip)
{
	input_unregister_device(chip->a_idev);
	input_free_device(chip->a_idev);
	sensor_remove_sysfs_interfaces(chip->als_dev, tmd2725_als_attrs,
				       ARRAY_SIZE(tmd2725_als_attrs));
	mutex_destroy(&chip->als_lock);
}
